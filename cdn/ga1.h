...
//        }
    ...
    // 初始化函数
    // 备注，暂时不用
    OurGA1(int size, int max_iterate, float elitism_rate, float mutation_rate) {
        ga_size = size;
    ...
    void decode() {
        ...
        if (!ChooseServer::serverID.empty()) {
            ...
            if (ga_run1.run(Graph::nodeCount,Graph::arcCount, ChooseServer::serverID) < INF) {
                write_result(ga_run1.getRoute(),ga_filename);
            }
